...
# Loading SEMs
Nest, we will load our SNP Effect Matrices and baseline file.
```{r get_files}
semFiles <- list.files("/users/g/k/gkenney/semplr/data/SEMs", 
                       pattern = ".sem", full.names = T)
semBaselines <- "/users/g/k/gkenney/semplr/data/SEMs/BASELINE/SEMs_baseline_norm.txt"
```
...
```{r score_motifs}
sempl_obj <- semMotifBinding(vr = vr, semFiles = semFiles, semBaselines = semBaselines)
sempl_obj
```

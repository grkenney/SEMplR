...
test_that(".binomSEMEnrich produces expected stats", {
  # typical case
  xs <- data.table::data.table(seqId = c("chr1:1", "chr1:2"),
                               SEM = "sem_id",
                               score = c(-2, 0),
                               scoreNorm = c(-1, 1),
                               index = 1,
                               seq = "AAAA")
  bg <- data.table::data.table(seqId = c("chr2:1", "chr2:2"),
                               SEM = "sem_id",
                               score = c(-2, -2),
                               scoreNorm = c(-1, -1),
                               index = 1,
  ...
  res_e <- data.table::data.table(SEM = "sem_id",
                                  pvalue = 0.75,
                                  n_bound = 1,
                                  n_bound_bg = 0)
...
test_that("enrichSEMs on 2 sites without specifying background", {
  ...
  expect_equal(e_a[e_a$n_bound > 0, ]$SEM, 
               c("IKZF1_HUMAN.GM12878", "ZNF18_HUMAN.HEK293"))
  expect_equal(e_a[e_a$n_bound > 0, ]$n_bound, 
               c(1, 1))
})
...
test_that("enrichSEMs on 2 sites with specifying background", {
  x <- GenomicRanges::GRanges(seqnames = c("chr12", "chr16"),
                              ranges = IRanges::IRanges(start = c(94136009,
                                                                ...
                                                                67637901)))
  bg <- GenomicRanges::GRanges(seqnames = c("chr6", "chr6"),
                              ranges = IRanges::IRanges(start = c(167127770,
                                                                  135097880),
                                                        end = c(167127770,
                                                                135097880)))
  b <- BSgenome.Hsapiens.UCSC.hg19::Hsapiens
  # score background seperately to validate results
  sb_bg <- scoreBinding(x = bg,
                        sem = sc,
                        genome = b)
  ...
  e_a <- enrichSEMs(x = sb_a, sem = sc, background = bg,
                    genome = BSgenome.Hsapiens.UCSC.hg19::Hsapiens)
  ...
  expect_equal(e_a[e_a$n_bound_bg > 0, ]$SEM, 
               c("IKZF1_HUMAN.GM12878", "M00967"))
  expect_equal(e_a[e_a$n_bound_bg > 0, ]$n_bound_bg, 
               c(1, 1))
})

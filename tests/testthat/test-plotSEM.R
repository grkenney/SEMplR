...
test_that(".definePlotSEMParams works for SNPEffectMatrices", {
  ...
  params_sem_a <- .definePlotSEMParams(sem = sem_obj, 
                                       motif = NULL) # without motif
  expect_equal(params_sem_a$sem_baseline, getBaseline(sem_obj))
  
  params_sem_a <- .definePlotSEMParams(sem = sem_obj, 
                                       motif = "MA0099.2") # with motif
  expect_equal(params_sem_a$sem_baseline, getBaseline(sem_obj))
...
test_that(".defineNucleotideColors chooses black text when motifSeq is NULL", {
  ...
  res_a <- .defineNucleotideColors(sem_mtx_long = sem_mtx_long, 
                                   motifSeq = NULL)
  expect_equal(colnames(res_a), c("sem_score", "motif_pos", 
                                  "bp", "text_color"))
...
test_that(".defineNucleotideColors fails when incorrect number of bases", {
  ...
  # too long
  expect_error(.defineNucleotideColors(sem_mtx_long = sem_mtx_long, 
                                       motifSeq = "AAAAAAAA"),
...
test_that("plotSEM returns a ggplot object", {
  plt_a <- plotSEM(sc, motif = "MA0099.2_HeLa",
                   motifSeq = "AAAAAAA",
                   highlight = 2)

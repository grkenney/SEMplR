...
test_that(".convertToSNPEffectMatrixCollection given collection", {
  expect_s4_class(.convertToSNPEffectMatrixCollection(sc), 
...
test_that(".convertToSNPEffectMatrixCollection given list", {
  x <- sems(sc)
  expect_s4_class(.convertToSNPEffectMatrixCollection(x), 
                  "SNPEffectMatrixCollection")
})
...
test_that(".convertToSNPEffectMatrixCollection given invalid input", {
  expect_error(.convertToSNPEffectMatrixCollection(TRUE), 
               "unable to convert object of class")
  expect_error(.convertToSNPEffectMatrixCollection(data.table::data.table()), 
               "unable to convert object of class")
})
...
test_that(".makeVariantId GRanges input", {
  x <- GenomicRanges::GRanges(seqnames = c("chr12"),
                              ranges = IRanges::IRanges(c(94136009)),
                              ref = "G", alt = "C")
  expect_equal(.makeVariantId(x, refCol = "ref", altCol = "alt"), 
               "chr12:94136009:G>C")
})


test_that(".makeVariantId GRanges input", {
  stp_a <- .semToPpm(sems(sc, "MA0099.2_HeLa"))
